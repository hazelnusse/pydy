...
C.abs_ang_vel = Vector(u0*B[1] + u1*B[2] + u2*B[3])
...
J_s = J.subs({q1s: 0, u0s: 0, u2s: 0})
evals = J_s.eigenvals()
eval_eqs = []
for i, ev in enumerate(evals.keys()):
    eval_eqs.append(Eq(Symbol("ev%d"%i), ev))
#print eval_eqs
critical_speed = [Eq(Symbol('cs'), solve(eval_eqs[0].rhs**2, [u1s])[0])]
##############################################################################
...
Mapping from time derivates of coordinates to generalized speeds.
    ...
    _x is an array/list in the following order:
        ...
        q2d:  Spin rate
"""
output_string += generate_function("qdot_to_u", qd_to_u_eqs, [q1, q0d, q1d,
    q2d], docstring=ds)
...
"""
output_string += generate_function("evals", eval_eqs, [u1], [g, r],
        docstring=ds)
