...
# Uncomment to study rolling torus
r2 = 0
...
u_rhs = [dot(C.ang_vel(N), A[i]) for i in (1,2,3)]
...
for k in kindiffs:
    kindiffs[k] = trigsimp(kindiffs[k])
    print k, '=', kindiffs[k]
...
for k in dependent_rates:
    dependent_rates[k] = trigsimp(dependent_rates[k])
    print k, '=', dependent_rates[k]
...
R_star = InertiaForce(CO)
print R_star
T_star = InertiaTorque(C)
print T_star
#print T_star
stop
